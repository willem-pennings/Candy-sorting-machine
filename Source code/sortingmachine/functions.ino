...
void initializeTCS34725() {
  ...
  if (tcs.begin()) {
    ...
    if(debug || debug_important){Serial.println(F("CRITICAL\tError while initializing the TCS34725 sensor! Program will continue nonetheless..."));}
  }
}

void initializeDRV2605() {
  // Initialize the vibrator controller IC
  drv.begin();

  // Configure the IC to trigger commands when sending a 'go' command
  drv.setMode(DRV2605_MODE_INTTRIG);

  // Set waveform - first argument in the setWaveform function is the index of the sequence (0, 1, 2 ... n)
  drv.setWaveform(0, 14); // "Strong buzz - 100%"
  drv.setWaveform(1, 15); // "750 ms Alert - 100%"
  //drv.setWaveform(1, 16); // "1000 ms Alert- 100%"

  // End waveform by sending sequence 0
  drv.setWaveform(2, 0);
}

void initializeSteppers() {
  stepper1.setMaxSpeed(4e3);
  stepper1.setAcceleration(5e5);
  stepper2.setMaxSpeed(5e3);
...
int getStepDistance(ItemColor nextcolor, ItemColor prevcolor) {
  ...
  if(candytype == mms) {
    switch(prevcolor) {
      case mm_brown: carouselPrevPos = containerPositions[0]; break;
      ...
      case mm_red: carouselPrevPos = containerPositions[5]; break;
      case unknown: carouselPrevPos = containerPositions[5]; break; // Unrecognized items are put in red container
    }
    switch(nextcolor) {
      case mm_brown: carouselNextPos = containerPositions[0]; break;
      ...
      case mm_red: carouselNextPos = containerPositions[5]; break;
      case unknown: carouselNextPos = containerPositions[5]; break; // Unrecognized items are put in red container
    }
  }
